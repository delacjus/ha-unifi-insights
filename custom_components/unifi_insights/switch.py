...
async def async_setup_entry(
    ...
    for site_id, routes in coordinator.data.get("policy_based_routes", {}).items():
        ...
        for route_id, route_data in routes.items():
            ...
            route_name = (
                route_data.get("name") or route_data.get("description") or route_id
            )
            _LOGGER.debug(
...
class UnifiPolicyBasedRouteSwitch(
    ...
    """Switch to enable or disable a policy-based traffic route."""
    ...
    def __init__(
        ...
        route_name = route_data.get("name") or route_data.get("description") or route_id
    ...
    def available(self) -> bool:
        """Return if the switch is available."""
        return bool(super().available and self._get_route_data())

    @property
    def is_on(self) -> bool:
        """Return True if the policy-based route is enabled."""
        route_data = self._get_route_data()
        return bool(route_data.get("enabled", False))
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        ip_addresses = route_data.get("ip_addresses")
        if ip_addresses is None:
            ip_addresses = route_data.get("ipAddresses", [])

        ip_ranges = route_data.get("ip_ranges")
        ...
        return {
            ...
            "next_hop": route_data.get("next_hop") or route_data.get("nextHop"),
        }
