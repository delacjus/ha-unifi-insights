...
class UnifiInsightsConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def _async_validate_local_connection(
        ...
        *,
        ...
        Returns (is_valid, error_code, via_protect_only). via_protect_only is
        True when the Network API had no sites and validation only succeeded
        via the Protect probe (e.g. a standalone Protect-only NVR/UNVR with
        no Network application) - callers use this to pick a distinguishing
        entry title.
        """
        ...
        # No Network sites - this console may not run the Network
        # application at all (e.g. a standalone Protect NVR). Probe Protect
        # before giving up, so Protect-only consoles can still be set up,
        # reauthed, or reconfigured with just an API key.
        # propagate_connection_errors=True lets a genuine connection failure
        # surface as cannot_connect instead of collapsing into invalid_auth.
        try:
        ...
        if last_auth_error:
            return False, "invalid_auth", False
        if last_parse_error:
            return False, "site_parse_error", False
        if last_conn_error:
            return False, "cannot_connect", False
        if last_not_found:
            return False, "api_unsupported", False
            ...
            try:
                ...
                if is_valid:
                    ...
                    return self.async_create_entry(
                        title=(
                            "UniFi Insights (Protect)"
                            if via_protect_only
                            else "UniFi Insights (Local)"
                        ),
                        data={
